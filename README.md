...
Each approach brings a different context model. PaC skills let users specify exactly which diffs, logs, and files to include. Fullsend relies on carefully curated documentation that humans write and maintain to reflect the codebase. Yeet adds a complementary layer: a knowledge graph of the entire repository — automatically derived from the code itself — that captures structural relationships (call graphs, type hierarchies, package boundaries, community clusters) and updates incrementally on every push.

Yeet is designed to integrate with PaC, reusing its adapter, provider, and Repository CR infrastructure. PaC skills and yeet agents can coexist on the same repository — skills handle lightweight prompt-driven tasks (formatting pipeline output, generating descriptions), while yeet agents handle tasks that benefit from deeper structural context. Yeet also builds on PaC's multi-provider foundation, supporting GitHub, GitLab, Bitbucket, and Gitea/Forgejo with multiple LLM providers — configured once on the Repository CR, shared by all agents.

## Comparison

| | Qodo Merge | PaC AI Skills | Fullsend | Yeet |
|---|---|---|---|---|
...
3. When a git event matches an agent's triggers, yeet spawns a sandboxed execution with filtered knowledge graph context
...
Agents describe their purpose in natural language. Yeet infers the right knowledge graph filtering strategy — no manual context configuration needed.
...
- [Getting Started](docs/getting-started.md) — deploy yeet locally with kind and test it end-to-end
